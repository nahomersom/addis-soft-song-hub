...
function Home() {
  ...
  // Get songs and loading state from Redux store
  const { songs, isPending } = useSelector((state: RootState) => state.songs);
        ...
        {isPending ? (
          ...
          <>
            ...
            {songs.length ? songs.map((song: SongResponse, index: number) => (
              <SongCard key={index}>
                <TableText><h6>{index + 1}</h6></TableText>
                <TableText><h6>{song.title}</h6></TableText>
                <TableText><h6>{song.artist}</h6></TableText>
                <TableText><h6>{song.album}</h6></TableText>
                <TableText><h6>{song.genre}</h6></TableText>
                <IconsContainer>
